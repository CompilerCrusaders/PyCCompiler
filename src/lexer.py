...
def reformat(c_file_path):

    with open(c_file_path, 'r') as file:  # opens file as readable doc under 'file'
...
def run(c_file_path):
    reformattedFile = reformat(c_file_path)
    ...
    with open('src/test/processedc.txt', 'r') as file:
        for line in file:
